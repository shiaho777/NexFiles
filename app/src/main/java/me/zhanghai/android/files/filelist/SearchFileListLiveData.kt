            ...
            try {
                path.search(options, INTERVAL_MILLIS) { paths: List<Path> ->
                    for (path in paths) {
                        ...
                        // The traversal prunes on name/size/time using only the cheap attributes;
                        // the mime-type filter is applied here against the precise value produced
                        // by loadFileItem(), so that content-sniffed types are matched correctly.
                        val mimeType = options.mimeType
                        if (mimeType != null && !mimeType.match(fileItem.mimeType)) {
                            continue
                        }
