...
class FileListAdapter(
    ...
    private val highlightColor: Int by lazy {
        ...
        if (ctx != null) {
            ...
            if (tv.type >= TypedValue.TYPE_FIRST_COLOR_INT) return@lazy tv.data
        }
        0x6633B5E5
    }
    ...
    fun replaceListAndIsSearching(
        ...
        val clear = this.isSearching != isSearching || this.searchOptions != searchOptions
    ...
    /**
     * Returns the file name as a [CharSequence], with matched substrings highlighted when a search
     * is active. Outside a search the plain name is returned with zero allocation, so there is no
     * overhead during normal browsing.
     */
    private fun highlightFileName(name: String): CharSequence {
        val options = searchOptions ?: return name
        val ranges = SearchOptions.highlightRanges(name, options)
        if (ranges.isEmpty()) {
            return name
        }
